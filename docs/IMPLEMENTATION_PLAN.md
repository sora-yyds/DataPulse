...
| M0-046 | 在真实 GitHub 远端配置并回读 protected `main`、squash merge 和 required check；用失败 PR 与直接推送否定验证证明门槛无法绕过；把 ruleset 与测试 PR 证据写入索引。**部分完成**：`m0 / main-protection` ruleset（pull_request squash-only、`m0 / pr-quick` 必查、non_fast_forward、无 bypass）已生效并回读，直接推送 GH013 拒绝、必查失败 PR 合并 HTTP 405 拒绝，正例经 ruleset 合并；**外部阻塞**：merge queue 仅对 organization 公共仓库可用（GitHub 官方文档；个人账户 REST 创建 merge_queue 返回 422），merge-group 聚合必查与失败 merge-group 否定验证不可执行。`verify:m0` 保持独立退出检查，不在 Wave 3 设成每个 PR 都必须通过 | M0-019；仓库所有者权限 | M |

当前 M0-016 Windows 阶段已让 Vitest、RTL、Storybook、Playwright 与 axe 各含真实产品断言，并以 `check:test-runners` 原子激活 TEST-RUNNERS 日常 gate；激活不等于任务或 M0 完成。M0-017 Windows 阶段又建立统一 fixture manifest、原始字节 hash／库存校验和 generated 固定 seed 合同，但尚未执行 M0-047 的大型语料生成、导入／分析闭环、干净 Ubuntu、GitHub Actions 或公开 Fork，因此 M0-CORPUS 只进入 `in_progress / partially_evidenced` 且保持未激活。M0-018 Windows 阶段已固定 `zh-CN`、`Asia/Shanghai`、字体、四主题 Token、Creator／Viewer 桌面与 Viewer 平板／手机视口和弱动效，以 `test:visual` 建立键盘焦点、200% 缩放、响应式与四主题视觉基线冒烟（24 run／18 passed／6 designed skips），并把 `check:test-runners` 扩展为六类 runner 6/6；它仍不代表完整 WCAG 2.2 AA 人工签署、固定视觉人工审查、HTTPS、四 Origin 或真实设备认证。M0-019 CI 阶段已落地 PR 快检、`merge_group` 已激活完整矩阵、main 复核、M0 退出候选与标签 release dry-run 五个 workflow 表面，并完成 PyYAML／`check:governance`／actionlint 本地校验；main 首次推送触发真实 `m0 / main-review`（Ubuntu 24.04）暴露 corepack shim 缺失与 designmd 版本输出平台差异，修复后复跑 run 31081289668 通过（`verify:pr` 6/6），证据已写入索引（`m0-ci-activation-m0-019-v2`）。merge queue required check、保护分支、失败 PR／merge-group 否定验证、公开 Fork 复现与干净 Ubuntu 矩阵仍未执行，因此任务与 CI-ACTIVATION 保持 `in_progress / partially_evidenced`。M0-020 已建立 release dry-run 供应链骨架：`release:dry-run` 生成 11 个 workspace 构建物暂存、`SHA256SUMS.txt` 与 SPDX 2.3 SBOM（453 个依赖包，许可证基线 2 个 NOASSERTION），`check:release-dryrun` 激活为 RELEASE-DRYRUN 日常 gate（Windows 本地 5/5，篡改自测全捕获）；标签触发 job 与干净公开 Fork 复现（M0-044）未执行，RELEASE-DRYRUN 保持 `in_progress / partially_evidenced`。M0-046 已完成真实远端部分验证：m0 / main-protection ruleset 在 main 生效并回读（pull_request squash-only、m0 / pr-quick 必查、non_fast_forward、无 bypass），直接推送 main 被 GH013 拒绝，必查失败 PR 合并被 HTTP 405 拒绝，正例 PR 经 ruleset 合并；证据已写入索引（m0-github-governance-ruleset-v1），GITHUB-GOVERNANCE 更新为 external_blocked / partially_evidenced。GitHub merge queue 仅对 organization 公共仓库（或 Enterprise Cloud 组织私有仓库）可用，个人账户 sora-yyds（account type User）下 REST 创建 merge_queue 规则返回 422 空错误，因此 merge queue 配置/回读、merge_group 日常完整矩阵必查与失败 merge-group 否定验证保持外部阻塞；GITHUB-GOVERNANCE 不能标为 passed，`verify:m0` 继续非零是正确结果。
