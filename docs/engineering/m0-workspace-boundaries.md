...
|---|---|---|---|
| `packages/domain` | `@datapulse/domain` | M0-010，以及 Creator／Viewer／API contract | 仅根 `.` export；领域 ID、稳定错误与 Result DTO 尚未实现 |
...
应用和服务不是可被其他 workspace 依赖的库，因此不声明 `exports`。两个共享包均为 `private: true`，只从 `dist/` 暴露列举的 JavaScript 与声明文件，不开放源码深导入或 `./*`。
...
依赖同时使用 `workspace:*` 与 TypeScript project reference 表达。当前跨 workspace 消费只验证包链接、构建产物和 exports map 可解析；业务源码 import 必须在对应 interface 真正落地的任务中原子加入，禁止发明 package marker、健康 DTO、成功空响应或 side-effect import。

Custom Connector 当前构建链验证 `@datapulse/api-contracts/connector-message` 可解析，并拒绝未公开的 contracts 根；`exports` 本身不能按消费者阻止同一包内的 `./http` 或 `./origin-policy`。因此“Connector 源码只能导入消息协议”仍是待 M0-007 静态强制的依赖规则，本任务不把约定冒充访问控制。Share API 当前验证 `./http` 与 `./origin-policy` 可解析。
...
本任务没有新增第三方依赖；TypeScript 与 Turbo 仍只作为根开发工具，产品运行时 bundle 增量为 0。

## 4. 延期目录

以下 module 在当前任务没有真实 interface，不创建空包：

- `packages/story-schema`、`packages/story-migrations`、`packages/metric-runtime`、`packages/themes`、`packages/renderer`：在 M0-011～015、M0-048、M0-049 出现正式消费者时创建；
...
Windows x64 使用固定 Node `24.19.0`、Corepack `0.35.0`、pnpm `11.20.0`、Turbo `2.10.8` 与 TypeScript `6.0.3` 验证冻结安装、6 个 workspace 构建和 workspace 契约。

当前执行面没有 WSL 发行版或容器运行时，因此干净 Ubuntu 未运行；统一人工边界复核也按项目所有者确认的阶段口径延期。两项不会被记为 Windows 的 skipped，也不会被表述为通过。`M0-005`、`M0-006` 与 `REPO-FOUNDATION` 继续保持进行中，日常 gate 不激活。
