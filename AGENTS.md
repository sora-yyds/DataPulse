...
当前已建立 M0-005 固定工具链、M0-006 的最小 workspace／独立 TypeScript 构建／workspace 契约、M0-007 依赖边界、M0-008 本地治理配置、M0-009 两类聚合器、M0-010 的 Windows 领域合同阶段实现、M0-011／012 的实验 Schema 与 Windows 对象校验基础、M0-013 的有界 Story Artifact Reader／未发布 `0.0.1 → 0.1.0` 复制迁移、M0-014 主题 Token／Design lint、M0-015 的最小二维 Renderer 与独立 Creator／Viewer React + Vite 验证页、M0-016 的 Windows Vitest／RTL／Storybook／Playwright／axe 五类真实 runner 与结构化日常检查、M0-017 的统一合成 fixture manifest／原始字节 hash／有界只读校验 seam 阶段实现、M0-018 的固定 `zh-CN`／`Asia/Shanghai`／字体／视口／弱动效视觉、键盘焦点、200% 缩放与四主题基线冒烟、M0-019 的 GitHub Actions CI 表面（PR 快检／merge_group 已激活完整聚合／main 复核／M0 退出候选／标签 release dry-run，本地校验完成且 main 推送的真实运行已复跑通过，但 merge queue required check、否定 PR 与公开 Fork 复现仍未执行）、M0-020 的 release dry-run 供应链骨架（构建物暂存／SHA256SUMS／SPDX 2.3 SBOM 与 `check:release-dryrun` 激活，标签触发与公开 Fork 复现未执行）、M0-021 的纯浏览器本地加密原语（RFC 4648 无填充 base64url、RFC 8785 JCS、Web Crypto CSPRNG 分块、purpose 绑定 AES-256-GCM AAD 与固定黄金向量）阶段实现、M0-022 的可审计 Argon2id WASM 依赖评估（hash-wasm 4.12.0 决策）与固定 `a2id-v1-64m-t3-p1` KDF profile（NFC 口令规范化、16B 盐、32B 输出、拒绝任意参数）及黄金向量阶段实现，并建立 `apps/device-probe` 独立设备探针页（KDF／AES／JCS／Fragment 固定向量、单次派生 ≤5s 计时与拒绝任意 KDF 参数）、M0-026 的不可导出设备密钥协议边界（ADR-0053 注册 `datapulse/device-bound-seal` purpose，`packages/local-storage` 提供设备密钥句柄、设备绑定 seal／open、持久存储请求与稳定错误，并已在真实 Chromium 通过否定验证（exportKey 拒绝、IndexedDB 仅存不可导出句柄、清除站点数据后不可恢复））、M0-051 的本地项目写入容量评估（4 GiB 全量备份载荷上界、project-envelope 信封开销投影、`navigator.storage.estimate` 配额 seam 与写入前双重容量拒绝）阶段实现、M0-052 的 OPFS 加密对象先写／IndexedDB 索引后写事务核心（pending→committed 原子翻转、崩溃点回滚、孤儿暂存清理、幂等恢复与最后一致索引，项目对象只存密文且拒绝原文件 Blob）阶段实现、M0-027 的 Project Repository 编排（open／commit／recover 状态机：所有 Story 读取经 Artifact Reader、迁移／解密／提交失败保留最后可读副本、Creator 保存与刷新恢复集成）阶段实现、M0-028 的导入探针契约与合成夹具（`packages/import-engine` 冻结 ADR-0050 准入限额、ImportRequest／进度／取消状态机与携带观测值的稳定拒绝错误；小型／常见／早期攻击夹具提交入库，200k 行窄表与 50 MB 宽表以固定 generator／seed 声明且输出不提交）阶段实现、M0-029 的导入准入资源估算与超限完整拒绝（`import-engine` 新增 CSV 数据行／列／非空单元格字节级扫描（表头不计入数据行）、`.xlsx` ZIP 中央目录／解压大小／压缩比检查、工作内存静态估算与 `IMPORT_ARCHIVE_INVALID` 封闭错误，`admitImport` 按冻结优先级完整拒绝并支持进度与取消）阶段实现、M0-030 的 Creator `LocalAnalysis` 契约与 `InProcessTestAdapter`（`packages/local-analysis` 唯一版本化消息 Schema 由受限 `./message` 子路径拥有，冻结 task ID／每请求 nonce／transferable 类型／长度／总字节上限与显式状态机，in-process adapter 通过同一 contract suite）阶段实现、M0-031 的 BrowserWorkerAdapter 与无运行时 import 的单文件模块 Worker 外壳（`@datapulse/local-analysis` 主线程在交付任何原始数据前从固定 Creator 资源路径预取固定 WASM 并做 SHA-256 校验，经新增 `bootstrap` 消息以结构化克隆传入 Worker；Worker 自身不 fetch／动态 import／importScripts／嵌套 Worker，Vite 打包产物为无 source map 的单文件模块并经静态检查；BrowserWorkerAdapter 与 InProcessTestAdapter 通过同一 run contract suite，取消／失败／完成均终止并释放 Worker 引用，WASM 拉取失败／哈希不匹配／Worker 不可达／终止均落为封闭 `LOCAL_ANALYSIS_WORKER_FAILED` 错误）阶段实现、M0-032 的真实浏览器 Worker CSP 网络通道否定与生命周期释放矩阵（`tests/worker-csp` 由 spec 自持本地 HTTP server 提供文档严格 CSP 与 Worker 响应自身携带的 CSP，真实 Chromium 验证 fetch／WebSocket／EventSource／动态 import／importScripts／嵌套 Worker 与 sendBeacon 全部不产生请求，canary Origin 零请求／零升级；BrowserWorkerAdapter 完成／取消／超时／失败四终态均 terminate 独占 Worker 并释放 transferable，新增 `test:worker-csp` 第八类 runner）阶段实现、M0-033 的 analysis-engine DuckDB-WASM 探针（`packages/analysis-engine` 第 17 个 workspace，以 DuckDB-WASM blocking API 对有界 Arrow IPC 流执行 `COUNT_ROWS` 与 `SUM(CAST(col AS DOUBLE))`，只返回与输入同序的版本化 MetricAccumulator；AnalysisInput 冻结 request ID、1 GiB Arrow 上限、1-64 指标请求与 SUM 严格标识符列名，不接受调用方表名，不读取原始行、不产出 Evidence/文案/Story 区块；engine 在完成/取消/失败路径均释放 DuckDB 连接，wasm 路径从本包 pnpm 布局解析且不引入 node:module resolver；单测 5 项覆盖黄金 COUNT/SUM 与 metric-runtime 逐值一致、输入校验/限额/SQL 注入列名/失败与取消稳定错误）阶段实现、M0-048 的正式 `1.0.0` Story Schema／原始字节 hash／Creator-Viewer 契约 fixture，以及 M0-049 的正式 `1.0.0` Metric Accumulator／Evaluation Plan Schema、固定 binary64 wire、最小 `COUNT_ROWS`／`SUM` 运行时与 Creator-Viewer 黄金结果 Windows 阶段实现；当前共 17 个 workspace。正式根运行时只加载正式 validator，正式历史的完整版本 tuple 与 validator mapping 从 manifest 生成；`0.x` 继续是隔离的未发布开发历史，公共 Reader 不接受它们，也不存在 `0.x → 1.0.0` 正式迁移边。公共 Reader 不开放可调限额或迁移路由，正式 Result 不暴露来源版本、目标版本或迁移步数。M0-015 页面提供正式 fixture → Reader → 共享 metric-runtime → Renderer 的只读探针，Creator 自 M0-027 起在本机可用时支持保存与刷新恢复；M0-016 的 production HTTP preview 与 Chromium／axe 自动近似不能替代固定视觉、键盘、缩放、完整 WCAG、HTTPS、四 Origin 或真实设备认证；M0-017 不执行生成器、导入／分析闭环或 M0-047 `test:corpus`；M0-018 的固定 Chromium 视觉／键盘／缩放冒烟仍是自动近似，不替代完整 WCAG 2.2 AA 人工签署、固定视觉人工审查或真实设备矩阵。Hono、未来正式版本迁移边、完整产品 UI／编辑器仍未建立；干净 Ubuntu 验证、真实 GitHub Actions merge queue required check、失败 PR／merge-group 否定验证与公开 Fork 复现仍未完成（main 推送复核已通过）；Windows 阶段结果不能推断为产品可用或关闭 M0-016、M0-017、M0-018、M0-021、M0-022、M0-026、M0-027、M0-028、M0-029、M0-030、M0-031、M0-032、M0-033、M0-049、M0-051、M0-052、TEST-RUNNERS、M0-CORPUS、CREATOR-VIEWER-CONTRACT。所有 pnpm 命令必须使用 Node `24.19.0`、Corepack `0.35.0` 和 `packageManager` 固定的 pnpm `11.20.0`，不匹配环境必须拒绝。
...
| 仓库根目录 | `corepack pnpm run build` | 修改任一 workspace 的源码、`package.json`、TypeScript reference、根 `tsconfig.json`、Story／Metric Runtime Schema、正式历史 manifest、生成器或 Turbo build 任务 | 进程退出 0，当前 15 个 workspace 均执行真实 `tsc --build`，产出独立 `dist` JavaScript、声明文件和 source map；Creator 与 Viewer 还分别执行 Vite 生产构建并生成独立 `dist/site`。`story-schema` 与 `metric-runtime` 先以 `--check` 拒绝陈旧的生成类型、Ajv standalone validator 和正式历史 metadata，并相对可信 merge-base 及 HEAD 可达、曾触碰各自正式目录的全部受保护提交核对版本、路径、原始字节数、SHA-256 与只追加语义；两个 workspace 的 Turbo build 均固定 `cache: false` 并透传 `DATAPULSE_MERGE_BASE`，防止缓存或长分支后续提交绕过历史检查；`story-schema` 的正式根运行时只加载正式 validator，`metric-runtime` 的公共运行时不加载 Ajv；`tsc` 发出的 `dist` JSON 只要求与源 Schema 语义等价，正式协议 hash 只取源文件原始字节；页面产物只是 M0-015 只读验证 bundle，不是完整可发布产品 |
| 仓库根目录 | `corepack pnpm run check:workspace` | 修改 workspace 目录、包名、依赖、exports、入口、构建配置、`tests/architecture/check-workspace.mjs` 或延期目录范围 | 先完成真实 Turbo build，再输出 `check=workspace-foundation`、`result=passed`、当前 Windows `574/574`、`failed=0`、`skipped=0`；验证 17 个必需 workspace、显式 exports、project references、产物、两端 Vite bundle／独立 JSON 资源与消费侧解析 |
| 仓库根目录 | `corepack pnpm run check:dependencies` | 修改任一 workspace manifest、TypeScript reference、`src`／包级脚本 import/re-export/dynamic import、package exports、完整方向策略、`pnpm-workspace.yaml`、`packages/domain` 公共合同／合同测试或 `tests/architecture/check-dependencies.mjs` | 进程退出 0，输出 `check=dependency-boundaries`、`result=passed`、当前 Windows 主断言 `3499/3499`、self-test `193/193`、domain contract `60/60`、`failed=0`、`skipped=0`、`cycles=0`；反向引用、循环、未声明／深导入、Renderer／Viewer／Connector／service 越界、Story Migrations 未授权 Schema subpath 或公共声明泄漏、Metric Runtime 非 Domain 运行时依赖、非法领域 ID、非法／重复／未知版本、不可区分 Result 或未登记 pnpm 图／build policy 必须非零 |
| 仓库根目录 | `corepack pnpm run test:unit` | 修改版本化 Story／Metric Runtime Schema、正式历史／生成类型／standalone validator／对象校验器、Story Artifact Reader／迁移、统一或领域 fixture／manifest（含 import-admission 小型／常见／攻击与 200k 窄表／50 MB 宽表生成集）、fixture checker、Metric Runtime merge／finalize／稳定错误优先级、Renderer、Creator／Viewer composition、domain ID／主题交叉契约、LocalAnalysis 唯一版本化消息 Schema／状态机／限额／bootstrap 与单文件 Worker 外壳、`tests/unit/`、`vitest.config.mts` 或 Vitest／Vite／Ajv／React／类型生成依赖 | 先构建 17 个 workspace，再在固定 Node 环境下从公开 `dist` seam 运行；进程退出 0，`passWithNoTests=false`、`allowOnly=false`，当前至少 `18 test files / 367 tests` 全部通过；覆盖 M0-011／012 既有合同、正式 Story 与 Metric Runtime `1.0.0` Schema／fixture 原始字节 hash、正式 Story 根 bundle 不含实验 validator、公共 Reader 拒绝 `0.x` 且不暴露迁移路由、Metric Runtime `COUNT_ROWS`／`SUM`、binary64 wire、固定 merge 顺序／错误优先级、65,536 accumulator 数量边界、Creator／Viewer 独立字节经 Reader → metric-runtime → composition → Renderer 的逐值一致、失败不渲染、四主题变量和 React 文本转义，以及统一 fixture catalog 的未知 set／generator／oracle、路径／realpath 别名逃逸、双向库存、重复身份、资源上限与稳定失败；同时覆盖 Argon2id `a2id-v1-64m-t3-p1` 黄金向量、Unicode NFC 规范化、盐长／口令长度／未知 profile 拒绝边界、local-storage 不可导出设备密钥（seal／open 往返、篡改／错误句柄／purpose 隔离、Node 下 IndexedDB 缺失与持久化不可用稳定错误）、写入容量评估（4 GiB 全量备份载荷边界、投影存储字节、quota／payload 双重拒绝与 Node 下配额 API 不可用稳定错误）、项目对象事务核心（OPFS 密文先写／IndexedDB 索引原子提交、pending 回滚、孤儿暂存清理、幂等恢复、最后一致索引、篡改／长度／哈希完整性拒绝与 Blob 输入拒绝）、Project Repository 状态机（commit 前经 Artifact Reader 预校验、open／commit／recover、迁移／解密／提交失败保留最后可读副本并回退）、导入探针契约（ADR-0050 冻结限额、带观测值的稳定拒绝错误、ImportRequest 运行时校验与进度／取消状态机）、LocalAnalysis 消息 Schema 运行时校验（task ID／nonce／transferable 类型／长度／总字节上限、消息体字节上限）、状态转换、InProcess 与 BrowserWorker 双 adapter 共享 contract suite（bootstrap 预取／SHA-256 校验／WASM 拉取失败／哈希不匹配／Worker 不可达／运行期终止封闭错误与取消释放）、analysis-engine DuckDB-WASM 探针（黄金 COUNT_ROWS／SUM 与 metric-runtime 逐值一致并回灌 evaluateMetric、小数／负值／空表、输入校验／限额矩阵、SQL 注入列名、NaN／空 SUM／缺列／恶意 Arrow／取消稳定错误与连续独立实例稳定）与 16 MiB Story Artifact 先验限制、真实 Uint8Array／Buffer／跨 realm、fatal UTF-8／BOM、未知／恶意样本、开发复制迁移、逐步／最终校验、数量／排名语境单个中文数词、失败不替换、Node 原生 ESM 与 Vite no-write。该入口本身不代表其余七类 runner、项目仓库原子提交、固定视觉／完整无障碍矩阵或跨平台 gate 通过 |
| 仓库根目录 | `corepack pnpm run test:component` | 修改 Creator／Viewer `App` 可观察状态、Renderer DOM、fixture fetch 边界、`tests/component/`、`vitest.component.config.mts` 或 RTL／jsdom 依赖 | 先构建 17 个 workspace，再以 jsdom、单 worker、`passWithNoTests=false`、`allowOnly=false` 运行；当前 `1 file / 2 tests` 全部通过，只 mock `fetch` 并验证两端正式独立字节显示标题、KPI `23`、范围、evidence 且无 alert；不冒充真实浏览器布局／Origin |
| 仓库根目录 | `corepack pnpm run test:storybook` | 修改 Renderer story、Storybook 配置／preview、主题／区块可观察输出、`vitest.storybook.config.mts` 或 Storybook／Vitest browser 依赖 | 先构建 17 个 workspace，再完成 Storybook 静态构建并以固定 Playwright Chromium 运行；当前 `1 file / 1 story test` 通过，`play` 验证标题、KPI `23`、范围、evidence，`addon-a11y` 以 `test=error` 接入；静态构建成功不能替代交互测试，chunk warning 不作为产品预算 |
| 仓库根目录 | `corepack pnpm run test:storage` | 修改 `packages/local-storage` 设备密钥生命周期、`playwright.storage.config.ts`、`tests/storage-e2e/`、`tests/storage-fixture/` 或 Playwright／Vite 依赖 | 先构建 17 个 workspace，再由 spec 自持 Vite dev server 在 `http://127.0.0.1`（安全上下文）真实 Chromium 运行；当前 `3 files / 13 tests` 通过：不可导出密钥（`extractable=false`、`exportKey` 拒绝）、seal／open 往返与篡改拒绝、句柄跨 reload 持久、IndexedDB 仅存不可导出句柄且无原始字节、清除站点数据后 `STORAGE_DEVICE_KEY_MISSING` 且旧密文永久不可开、持久存储请求返回布尔或稳定错误且不虚报持久；项目对象事务核心（OPFS 密文先写、IndexedDB 索引原子提交、崩溃点回滚、孤儿清理、最后一致索引与清除站点数据后不可恢复）；Project Repository 真实浏览器往返（commit→open、reload 后恢复、密文篡改回退最后可读副本、OPFS 只存密文）；spec 内服务器不触碰 Creator／Viewer 的 4173/4174 双 webServer 生命周期合同 |
| 仓库根目录 | `corepack pnpm run test:worker-csp` | 修改 `packages/local-analysis` Worker 生命周期、Worker CSP 否定测试、`playwright.worker-csp.config.ts`、`tests/worker-csp/`、`tests/worker-csp-fixture/` 或 Playwright 依赖 | 先构建 17 个 workspace，再由 spec 自持本地 HTTP server 在 `http://127.0.0.1` 真实 Chromium 运行；当前 `1 file / 3 tests` 通过：文档严格 CSP 下页面 sendBeacon 与适配器 WASM 预取被拦截；Worker 响应自身携带的 CSP 拒绝 fetch／WebSocket／EventSource／动态 import／importScripts／嵌套 Worker，canary Origin 零请求／零升级且主 Origin 无 `/prohibited` 请求；BrowserWorkerAdapter 完成／取消／超时／失败四终态均 terminate 独占 Worker 并释放 transferable；bundle 静态否定在真实浏览器复验。spec 内服务器不触碰 Creator／Viewer 的 4173/4174 双 webServer 生命周期合同 |
| 仓库根目录 | `corepack pnpm run test:e2e` | 修改 Creator／Viewer 生产页面、Vite 构建、Playwright config、浏览器 preview 生命周期合同、`tests/e2e/` 或 Playwright 依赖／浏览器版本 | 先构建 17 个 workspace，在各 app cwd 由 Playwright 直接拥有的固定 Node／Vite CLI 进程上，以 `127.0.0.1:4173/4174`、`--strictPort`、`reuseExistingServer=false` 启动独立 production HTTP preview；当前 Chromium `2/2` 通过并验证两端标题、应用身份、KPI `23`、范围、evidence、无 alert，连续 E2E／axe 运行后端口必须释放；仅为自动近似，不证明 HTTPS、四 Origin、Safari／微信或完整产品 E2E |
| 仓库根目录 | `corepack pnpm run test:a11y` | 修改页面语义、Renderer、主题 Token、Playwright axe config、`tests/a11y/` 或 axe／Playwright 依赖 | 先构建 17 个 workspace 并启动同一对严格 production HTTP preview；当前 Creator／Viewer `2/2` 通过，不排除产品内容、不禁用规则，要求 axe 实际评估结果非零且 `violations=0`；这不是完整 WCAG 2.2 AA、键盘、焦点、缩放、对比度人工签署或真实设备认证 |
| 仓库根目录 | `corepack pnpm run test:visual` | 修改页面语义／布局、Renderer、主题 Token、Playwright 视觉配置、`tests/visual/`、视觉基线或 Playwright 依赖 | 先构建 17 个 workspace，再以与 E2E／axe 相同的严格 production HTTP preview 与各 app cwd 直启 Node／Vite CLI 运行；固定 `zh-CN`、`Asia/Shanghai`、弱动效（`openStory` 显式 `emulateMedia` 兜底）与四主题 Token，当前 `24 run / 18 passed / 6 designed skips`，覆盖字体回退链、`:focus-visible` 焦点环、200% 缩放无水平溢出且核心内容不重叠、响应式不溢出和四主题视觉基线；Playwright 快照仅是固定 Chromium 自动近似，不证明完整 WCAG 2.2 AA、键盘／焦点人工签署、缩放对比度人工审查或真实设备 |
| 仓库根目录 | `corepack pnpm run check:test-runners` | 修改任一八类 runner 根脚本、测试配置／产品断言、`scripts/check-test-runners.mjs`、TEST-RUNNERS activation 或相关依赖 | 顺序真实运行 `test:unit`、`test:component`、`test:storybook`、`test:storage`、`test:worker-csp`、`test:e2e`、`test:a11y`、`test:visual`；输出单行 `check=test-runners`、`result=passed`，当前结构化断言 `8/8`、`failed=0`、`skipped=0`，并回显本次 gate／nonce；任一子入口失败时最终非零且不静默跳过。Windows 通过不关闭 M0-016／TEST-RUNNERS／M0-018／M0-032 |
...
| 仓库根目录 | `corepack pnpm run release:dry-run` | 修改 `scripts/release-dry-run.mjs`、workspace dist 布局、release 产物合同或 `release-dry-run.yml` | 先构建 17 个 workspace，再按冻结合同暂存 `release/dry-run/<version>/`（17 个 workspace、排除 `node_modules`／`.turbo`／`*.tsbuildinfo`），生成 `manifest.json`、`SHA256SUMS.txt` 与 SPDX 2.3 `sbom.spdx.json`（469 个依赖包、purl 引用、`Unknown` 许可证归一化为 `NOASSERTION`）；进程退出 0 且输出 `result=passed` |
| 仓库根目录 | `corepack pnpm run check:release-dryrun` | 修改任一 release dry-run 脚本、产物合同、workspace dist 布局、根 `package.json` scripts、SPDX 生成逻辑或 `release-dry-run.yml` | 先真实 `turbo run build`，再重新生成并验证 workspace 契约（当前 17 个）、违禁条目、`SHA256SUMS` 覆盖与值、SBOM 形状／workspace 与依赖覆盖、NOASSERTION 许可证基线（当前 `@google/design.md@0.4.0`、`spawndamnit@3.0.1`）、文件校验和与再生成确定性；`--self-test` 篡改／删除／违禁／许可证缺失变异全捕获；进程退出 0，当前 Windows 本地 5/5。标签触发 job 与公开 Fork 复现（M0-044）未执行，RELEASE-DRYRUN 仍为 `in_progress / partially_evidenced` |
| 仓库根目录 | `corepack pnpm run check:fixtures` | 修改统一 fixture Schema／manifest、任一受控子 manifest 或 fixture（含 import-admission 小型／常见／攻击与 200k 窄表／50 MB 宽表生成集）、Creator／Viewer fixture 副本、fixture checker／单测、M0-CORPUS 状态或相关证据 | 进程退出 0，输出 `check=fixture-manifest`、`result=passed`、当前 `7` 个逻辑集／`20` 个 artifact／`2` 个 generated 集，主断言 `521/521`、恶意 self-test `30/30`、`failed=0`、`skipped=0`；核对具体 primitive／errorCode oracle、固定 generator policy、POSIX／Windows 保留名和 8.3 别名、symlink／junction、realpath 唯一性、原始 bytes／SHA-256、双向库存完整性和有界读取，且不执行生成器。该入口不等于 M0-047 `test:corpus`、大型 CSV／XLSX 生成、导入／分析、Ubuntu、GitHub Actions 或公开 Fork 已运行 |
