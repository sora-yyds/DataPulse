...
| 仓库根目录 | `corepack pnpm run test:storage` | 修改 `packages/local-storage` 设备密钥生命周期、`playwright.storage.config.ts`、`tests/storage-e2e/`、`tests/storage-fixture/` 或 Playwright／Vite 依赖 | 先构建 14 个 workspace，再由 spec 自持 Vite dev server 在 `http://127.0.0.1`（安全上下文）真实 Chromium 运行；当前 `2 files / 9 tests` 通过：不可导出密钥（`extractable=false`、`exportKey` 拒绝）、seal／open 往返与篡改拒绝、句柄跨 reload 持久、IndexedDB 仅存不可导出句柄且无原始字节、清除站点数据后 `STORAGE_DEVICE_KEY_MISSING` 且旧密文永久不可开、持久存储请求返回布尔或稳定错误且不虚报持久；项目对象事务核心（OPFS 密文先写、IndexedDB 索引原子提交、崩溃点回滚、孤儿清理、最后一致索引与清除站点数据后不可恢复）；spec 内服务器不触碰 Creator／Viewer 的 4173/4174 双 webServer 生命周期合同 |
