...
| 仓库根目录 | `corepack pnpm run release:dry-run` | 修改 `scripts/release-dry-run.mjs`、workspace dist 布局、release 产物合同或 `release-dry-run.yml` | 先构建 11 个 workspace，再按冻结合同暂存 `release/dry-run/<version>/`（11 个 workspace、排除 `node_modules`／`.turbo`／`*.tsbuildinfo`），生成 `manifest.json`、`SHA256SUMS.txt` 与 SPDX 2.3 `sbom.spdx.json`（453 个依赖包、purl 引用、`Unknown` 许可证归一化为 `NOASSERTION`）；进程退出 0 且输出 `result=passed` |
| 仓库根目录 | `corepack pnpm run check:release-dryrun` | 修改任一 release dry-run 脚本、产物合同、workspace dist 布局、根 `package.json` scripts、SPDX 生成逻辑或 `release-dry-run.yml` | 先真实 `turbo run build`，再重新生成并验证 workspace 契约（当前 11 个）、违禁条目、`SHA256SUMS` 覆盖与值、SBOM 形状／workspace 与依赖覆盖、NOASSERTION 许可证基线（当前 `@google/design.md@0.4.0`、`spawndamnit@3.0.1`）、文件校验和与再生成确定性；`--self-test` 篡改／删除／违禁／许可证缺失变异全捕获；进程退出 0，当前 Windows 本地 5/5。标签触发 job 与公开 Fork 复现（M0-044）未执行，RELEASE-DRYRUN 仍为 `in_progress / partially_evidenced` || 仓库根目录 | `corepack pnpm run check:fixtures` | 修改统一 fixture Schema／manifest、任一受控子 manifest 或 fixture、Creator／Viewer fixture 副本、fixture checker／单测、M0-CORPUS 状态或相关证据 | 进程退出 0，输出 `check=fixture-manifest`、`result=passed`、当前 `4` 个逻辑集／`12` 个 artifact／`0` 个 generated 集，主断言 `312/312`、恶意 self-test `26/26`、`failed=0`、`skipped=0`；核对具体 primitive／errorCode oracle、固定 generator policy、POSIX／Windows 保留名和 8.3 别名、symlink／junction、realpath 唯一性、原始 bytes／SHA-256、双向库存完整性和有界读取，且不执行生成器。该入口不等于 M0-047 `test:corpus`、大型 CSV／XLSX 生成、导入／分析、Ubuntu、GitHub Actions 或公开 Fork 已运行 |
