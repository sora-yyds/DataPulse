...
import { evaluateViewerMetric } from "./main.js";
 ...
 */
export function composeViewerStory(
  blueprint: ValidatedStoryBlueprint,
  ...
  const evaluationResult = evaluateViewerMetric(
...
/**
 * Viewer 浏览器与测试共用的 fail-closed seam。只有正式 Story Reader 和
 * 确定性指标 composition 都成功时，调用方才会取得可交给 Renderer 的候选。
 */
export function prepareViewerStory(
  ...
  const compositionResult = composeViewerStory(
    storyResult.value,
    metricFixtureBytes,
  );
